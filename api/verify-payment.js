...
export default async function handler(req, res) {
  ...
  const buyerHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0f1e; color: #fff; border-radius: 12px; overflow: hidden;">
      <div style="background: linear-gradient(135deg, #1e3a8a 0%, #0ea5e9 100%); padding: 32px 24px; text-align: center;">
        <h1 style="margin: 0; font-size: 28px; font-weight: 800; letter-spacing: -0.5px;">🎉 Your Purchase is Confirmed!</h1>
        <p style="margin: 8px 0 0; opacity: 0.85; font-size: 15px;">Thank you for buying from Buddy Tezz AI</p>
      </div>
  ...
  const adminHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f9f9f9; border-radius: 8px;">
      ...
      <table style="width:100%; border-collapse:collapse; background:#fff; border-radius:6px; overflow:hidden;">
        ...
        <tr><td style="padding:12px 16px; font-weight:bold; color:#374151;">Razorpay Order ID</td><td style="padding:12px 16px; color:#111827;">${razorpay_order_id}</td></tr>
        <tr style="background:#f3f4f6;"><td style="padding:12px 16px; font-weight:bold; color:#374151;">Payment ID</td><td style="padding:12px 16px; color:#111827;">${razorpay_payment_id}</td></tr>
        <tr><td style="padding:12px 16px; font-weight:bold; color:#374151;">Purchased At</td><td style="padding:12px 16px; color:#111827;">${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} IST</td></tr>
      </table>
    </div>
  `;
  ...
  try {
    ...
    // Payment was verified — log email failure but still return success
    console.error('Email delivery failed after verified payment:', emailError);
    return res.status(200).json({
      message: 'Payment verified. Email delivery failed — please contact buddytezzai@gmail.com with your payment ID.',
